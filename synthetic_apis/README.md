...
| [Dashboard Alerts](dashboard_alerts.md)     | `conversation_resolved`, `resolve_dashboard_header` (from the header's `resolution` object) | `dashboard_header`                                                                        | Catalog of every priority 5 and 6 alert that can appear in the dashboard header, with names, titles, escalation phases, and clearing rules.                        |
...
| [Radar Devices](radar.md) | `set_radar_room`, `set_radar_subregion`, `delete_radar_subregion`, `set_radar_config`, `submit_radar_fall_feedback` | `radar_room`, `radar_subregions`, `radar_subregion_behaviors` | Fully manage radar devices (Vayyar Care, Pontosense, Nobi, AeroSense Assure) to detect falls and occupancy. |
